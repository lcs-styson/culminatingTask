...
class ParameterController: NSViewController {
    ...
    // This is called when the slider's value changes
    @IBAction func thicknessValueChanged(_ sender: NSSlider) {
        
        
        print("Thickness is now is now: \(sender.integerValue)")

        // Update in the sketch
        childSketch?.sketch.size = sender.integerValue
    }
    ...
    override func prepare(for segue: NSStoryboardSegue, sender: Any?) {
        ...
        if let vc = segue.destinationController as? ViewController {
            ...
            vc.sketch.size = thicknessSlider.integerValue
